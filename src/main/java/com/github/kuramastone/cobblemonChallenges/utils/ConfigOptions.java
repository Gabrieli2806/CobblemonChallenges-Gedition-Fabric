...
public class ConfigOptions {
    ...
    @YamlObject("permission-challenges.no-permission-challenge-item")
    ...
    private void loadGuiConfigPerChallenge() {
        ...
        for (File yamlFile : YamlConfig.getYamlFiles(parentFolder)) {
            ...
            YamlConfig guiSection = config.getSection("gui");
            if (guiSection == null) {
                // CobbleChallengeMod.logger.warn("No GUI section found in challenge file: {}", yamlFile.getName());
                continue;
            }
            
            GuiConfig gui = GuiConfig.load(guiSection);
            challengeConfigs.put(name, gui);
            // CobbleChallengeMod.logger.info("Loaded GUI config for challenge list '{}' with {} ingredients", name, 
// gui.getIngredients() != null ? gui.getIngredients().size() : 0);
        }
